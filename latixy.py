...
latitude2=np.zeros((lat_size,lon_size))
for i in range(lat_size):
    latitude2[i,:]=latitude[i]
...
ncfile = Dataset('latixy.nc',mode='w',format='NETCDF4_CLASSIC') # Decide here the name of your file 
print(ncfile)
...
Q = ncfile.createVariable('latixy',np.float64,('lat','lon')) # note: unlimited dimension is leftmost
lat[:]=latitude
lon[:]=longitude

# Write latitudes, longitudes.
data_arr=np.zeros((lat_size,lon_size))
data_arr=latitude2 #np.flip(ID,0)
